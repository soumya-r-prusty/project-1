...
function validateForm(formData) {
  ...
  const pnum = $('#pnum').val();
  const gender = $('input[name = gender]:checked', '#formsubmission').val();
  const stret = $('#street').val();
  const ciVillage = $('#ci-village').val();
  const post = $('#post').val();
  ...
  const checkpassbox = $('#check-pass-box').val();



  const fnameErr = $('fname-error');
  const lnameErr = $('lname-error');
  const dobErr = document.getElementById('dob-error');
  const emailErr = document.getElementById('email-error');
  const pnumErr = document.getElementById('p-error');
  const genderErr = document.getElementById('gender-error');
...
function validFname(){
  ...
  if(username.length === ""){
    ...
    $('#fname-error').show();
    $('#fname-error').html("** length of username must be between 3 and 10");
    fnameerror = false;
    return false;
  }
  else{
...
$(document).ready(function () {
  ...
  function readFormData() {
    ...
    formData.pnum = $('#pnum').val();
    // formData.gender = $('input[name = gender]:checked', '#formsubmission').val();
    formData.street = $('#street-no').val();
    formData.ciVillage = $('#ci-village').val();
    formData.post = $('#post-code').val();
    formData.state = $('#state').val();
    formData.country = $('#country').val();
    formData.region = $('#region').val();
    // formData.file = $('#file')[0].files[0];
  ...
  function resetForm() {
    ...
    $("input[type=text], input[type=radio],input[type=email],input[type=date],input[type=number],input[type=file], input[type=checkbox], input[type=password]").val('');
  }
  ...
  $('body').on('click', '#Edit', function () {
    ...
    $('#pnum').val('' + formData.pnum);
    $('#street-no').val('' + formData.street);
    $('#ci-village').val('' + formData.ciVillage);
    $('#post-code').val('' + formData.post);
    $('#state').val('' + formData.state);
    $('#country').val('' + formData.country);
    $('#region').val('' + formData.region);
    // $('#file').val(''+formData.file);
  ...
  $("#formsubmission").on('submit', function (event) {
   ...
   validFname();
    ...
    if (validateForm(formData)) {
      if (!selectRow) {
        $('#table tbody').append('<tr><td>' + formData.fname + '</td><td>' + formData.lname + '</td><td>' + formData.email + '</td><td>' + formData.dob + '</td><td>' + formData.pnum + '</td><td><button class="green" id= "Edit">  Edit  </button></td><td><button class="red del">Delete</button></td></tr>');
        resetForm();
        isTrue = true;
      }
      else {
